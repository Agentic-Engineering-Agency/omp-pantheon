...
Commands are journaled before execution. Every run records the canonical persisted OMP owner session file in private state; `--no-session` cannot start a run. Positive goal/verification evidence and `agent_end` attempt/continuation decisions are evaluated against the context that emitted each event, so switching the loaded runtime to another session cannot transfer ownership. Mutating tool results remain project-wide invalidation signals: a write from any session sharing the canonical project root advances the artifact revision and resets every gate. Project identity is canonicalized through the filesystem before state derivation and ownership checks, so symlink aliases share one run. Project attachment prepares a complete controller/store/worker context before publishing it atomically; a concurrent event is handled by either the previous complete context or the next complete context, never a mixture. Each command is bound to one autonomy run, one canonical project root, the exact absolute owner session file, and a finite attempt bound. When an attempt ends without current gate evidence, the extension durably schedules the next continuation for that same file-backed owner session.
